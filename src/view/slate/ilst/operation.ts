...
const op = {
  is: {
    list( node: Node ) {
      return node.type === EDITOR_ELEMENT_LIST.TYPE
    },
    item( node: Node ) {
      return node.type === EDITOR_ELEMENT_LIST.ITEM
    },
    nested( editor: ReactEditor, at: Path ) {
      return Array.from( Editor.levels( editor, { at } ) )
        .slice( 0, -1 )
        .some( ( [ node ] ) => op.is.list( node ) )
    },
  },
}
...
function isNested( editor: ReactEditor, path: Path ) {
  return Array.from( Editor.levels( editor, { at: path } ) )
    .slice( 0, -1 )
    .some( ( [ node ] ) => node.type === EDITOR_ELEMENT_LIST.TYPE )
}
...
function toggle( editor: ReactEditor, details: EditorElementListDetails = { variant: 'unordered' } ) {
  needUpdate( editor, details ) ? update( editor, details ) : insert( editor, details )
}
...
function insertAt( editor: ReactEditor, details: EditorElementListDetails, at: Path ) {
  ...
  if( Element.isElement( nest ) && match( nest ) ) {
    ...
    Transforms.setNodes(
      editor,
      { type: EDITOR_ELEMENT_LIST.ITEM },
      { at, match: itemMatcher, mode: 'lowest' },
    )
    Transforms.wrapNodes( editor, { type: EDITOR_ELEMENT_LIST.TYPE, ...details, children: [] } )
  }
}
