import { Action, Event, Query } from '@brainote/domain'
import {
  ConnectedSocket,
  MessageBody,
  OnGatewayConnection,
...
export class IoGateway implements OnGatewayConnection {
  ...
  handleAction(@MessageBody() action: Action.Any, @ConnectedSocket() socket: Socket) {
  ...
  handleQuery(@MessageBody() query: Query.Any, @ConnectedSocket() socket: Socket) {
