...
function needUpdate(editor: ReactEditor, details?: EditorElementListDetails) {
  ...
  return (
    top &&
    details &&
    (top[0].variant !== details.variant || top[0].style !== details.style)
  )
}
...
function update(editor: ReactEditor, details: EditorElementListDetails) {
  console.debug('Updating list at selection with : ', editor.selection, details)
  const [top] = Editor.nodes(editor, { match })
  if (top) {
...
function merge(editor: ReactEditor, at: Path, to: Path) {
  console.debug('Merging lists : ', at, to)
...
function insert(editor: ReactEditor, details: EditorElementListDetails) {
  console.debug(
    'Inserting list at selection with : ',
    editor.selection,
    details,
  )
...
function insertAt(
  ...
  console.debug('Inserting list at with : ', at, details)
...
function remove(editor: ReactEditor) {
  console.debug('Removing list at selection : ', editor.selection)
